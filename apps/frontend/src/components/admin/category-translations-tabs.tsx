...
interface CategoryTranslationsTabsProps {
  ...
  onTranslationDataChange?: (data: Record<string, { name: string; description: string }>) => void;
}

export interface CategoryTranslationsTabsRef {
  resetChanges: () => void;
  validateAll: () => { isValid: boolean; errors: Record<string, string[]> };
  getTranslationData: () => Record<string, { name: string; description: string }>;
}

export const CategoryTranslationsTabs = forwardRef<CategoryTranslationsTabsRef, CategoryTranslationsTabsProps>(({
  categoryId,
  defaultName = '',
  defaultDescription = '',
  creationMode = false,
  ...
  useEffect(() => {
    ...
    activeLangs.forEach((lang) => {
      ...
      // Only use defaultName/defaultDescription for default language in creation mode, or if there's an existing translation
  ...
  }, [translations, languages, defaultName, defaultDescription, creationMode]);
  ...
  // Validation function
  const validateTranslation = (data: { name: string; description: string }): { 
    hasErrors: boolean; 
    errors: { name?: string };
...
CategoryTranslationsTabs.displayName = 'CategoryTranslationsTabs';
